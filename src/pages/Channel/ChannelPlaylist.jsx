...
import { createPlaylist, getUserPlaylists } from "../../store/Slices/playlistSlice";
...
export default function ChannelPlaylist() {
    ...
    const authId = useSelector((state) => state.auth?.userData?._id); // user which is logged in
    const userId = useSelector((state) => state.user?.userProfile?._id); // user whose channel we are visiting

    const { handleSubmit, register, formState: {errors} } = useForm();

    const [openCreatePlaylist, setOpenCreatePlaylists] = useState(false);
